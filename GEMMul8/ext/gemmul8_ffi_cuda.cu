...
static ffi::Error EnsureWorkspace(
    cudaStream_t stream,
  ...
  // For simplicity we leak the old buffer (rare if shapes are static under jit).
  void* new_ptr = nullptr;
  cudaError_t st = cudaMallocAsync(&new_ptr, required_bytes, stream);
  if (st != cudaSuccess) {
    return ffi::Error::Internal("cudaMallocAsync failed for GEMMul8 workspace.");
  }
...
ffi::Error Gemmul8GemmF64Impl(
  ...
  if (kB != k) {
    return ffi::Error::InvalidArgument("Inner dimension mismatch between A and B.");
  }
  ...
  // If you pass JAX arrays, use jnp.asfortranarray-like layouts or accept a transpose convention.
  ...
  // UseExtraWorkspace is a compile-time bool in GEMMul8. We dispatch both cases.
  size_t total_ws = 0;
  if (use_extra_workspace) {
    total_ws = gemmul8::workSize</*is_Complex=*/false, /*UseExtraWorkspace=*/true>(
        (size_t)m, (size_t)n, (size_t)k,
        (unsigned)num_moduli,
        (bool)(enable_skip_scalA != 0),
        (bool)(enable_skip_scalB != 0),
        &workSizeA, &workSizeB);
  } else {
    total_ws = gemmul8::workSize</*is_Complex=*/false, /*UseExtraWorkspace=*/false>(
        (size_t)m, (size_t)n, (size_t)k,
        (unsigned)num_moduli,
        (bool)(enable_skip_scalA != 0),
        (bool)(enable_skip_scalB != 0),
  ...
  if (use_extra_workspace) {
    (void)gemmul8::gemm<double, /*UseExtraWorkspace=*/true>(
        ...
        (size_t)m, (size_t)n, (size_t)k,
        &alpha,
        (const double*)A.data(), (size_t)lda,
        (const double*)B.data(), (size_t)ldb,
        &beta,
        (double*)C->data(), (size_t)ldc,
        (unsigned)num_moduli,
        (bool)(fastmode != 0),
        work_rem,
        workA,
...
XLA_FFI_REGISTER_HANDLER(ffi::GetXlaFfiApi(), "gemmul8_gemm_f64", Gemmul8GemmF64);
